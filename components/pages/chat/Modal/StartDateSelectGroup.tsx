...
interface EndDateSelectProps {
  ...
  handleStartDate: (key: DateKeyType, date: string) => void;
}
const currentYear = new Date().getFullYear();
const YEAR = Array.from({ length: 4 }, (_, index) => currentYear + index);

export default function StartDateSelectGroup({
  startDate,
  handleStartDate,
}: EndDateSelectProps) {
  ...
  return (
    <div className="flex flex-col w-full gap-2">
      ...
      <div className="flex gap-2">
            ...
            {YEAR.map((year) => (
              <Select.Option value={year.toString()}>
                <div
                  ...
                  onClick={() => handleStartDate('year', year.toString())}
                  aria-hidden="true"
                >
                  {year}
                </div>
        ...
        <Select>
          <Select.Trigger className="border rounded-[10px] px-5 py-4">
            <Select.Label defaultValue={startDate.month} />
          </Select.Trigger>
          <Select.OptionList>
            {MONTH.map((month) => (
              <Select.Option value={month} key={month}>
                <div
                  ...
                  tabIndex={0}
                  className="w-full p-4"
                  onClick={() => handleStartDate('month', month)}
                  aria-hidden="true"
                >
                  {month}
                </div>
        ...
        <Select>
          <Select.Trigger className="border rounded-[10px] px-5 py-4 ">
            <div>
              {dateArray.includes(startDate.date) ? startDate.date : lastDate}
            </div>
          </Select.Trigger>
          <Select.OptionList>
            {dateArray.map((date) => (
              <Select.Option value={date} key={date}>
                <div
                  ...
                  tabIndex={0}
                  aria-hidden
                  onClick={() => handleStartDate('date', date)}
                >
                  {date}
                </div>
              </Select.Option>
