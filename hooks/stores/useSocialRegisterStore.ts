...
export interface SocialRegisterStore {
  step: number;
  allCheked: boolean;
  checkList: boolean[];
...
export const useSocialRegisterStore =
  createWithEqualityFn<SocialRegisterStore>()(
    devtools((set) => ({
      step: 1,
      allCheked: false,
      ...
      imageFile: null,
      setStep: (curStep: number) =>
        set((store) => ({ ...store, step: curStep })),
